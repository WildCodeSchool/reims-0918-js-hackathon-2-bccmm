export const questionQuizImp3D = [
  {
    id: 1,
    question: "Que peut-on faire avec une imprimante 3D ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://i.all3dp.com/wp-content/uploads/2018/10/05204941/flashhog.jpg",
          explication:
            "Un joli hérisson en plastique pour stocker ses clefs USB !"
        },
        content: "Un hérisson"
      },

      {
        type: {
          imageUrl:
            "https://i.all3dp.com/wp-content/uploads/2018/09/04130906/3D_printed_super_heros_formbyte_large-1.jpg",
          explication: "Des portes-clés aux motifs personnalisés !"
        },
        content: "Un porte-clés"
      },
      {
        type: {
          imageUrl:
            "https://i.all3dp.com/wp-content/uploads/2018/10/05205851/pencil-holde-772x434.jpg",
          explication:
            "On peut même faire un pot à crayon hérisson(On adore les hérissons)"
        },
        content: "Pot à crayon"
      }
    ]
  },
  {
    id: 2,
    question: "Quels matériaux peut-on utiliser pour imprimer en 3D ?",
    answers: [
      {
        type: {
          imageUrl:
            "http://fichier3d.fr/wp-content/uploads/2016/08/Pokemon-go-iphone-3D-printing-impression-cults-3D-fichier-3D-lanceur-figurine-pikachu-1.jpg",
          explication:
            "C'est le premier matériau utilisé en imprimerie 3D car il peut fondre et se durcir selon la température."
        },
        content: "Le plastique"
      },

      {
        type: {
          imageUrl: "https://www.3dnatives.com/wp-content/uploads/aly.jpg",
          explication:
            "Le métal est utilisé en usine principalement, on peut faire des pièces mécaniques par exemple."
        },
        content: "Le métal"
      },
      {
        type: {
          imageUrl:
            "https://2.bp.blogspot.com/-QhslRdNBRqs/WU9960M0bAI/AAAAAAAAEfw/DV2Ovk49tXwUAizUAGeRWZxLimZBqhGdwCLcBGAs/s1600/DSCN4561%2B%25282%2529.JPG",
          explication:
            "Et oui, en cuisine aussi on peut utiliser l'impression 3D, le chocolat fond à la chaleur et se durcit quand il fait froid, comme le plastique."
        },
        content: "Le chocolat"
      }
    ]
  },
  {
    id: 3,
    question: "Dans quels secteurs d'activités peut-on imprimer en 3D ?",
    answers: [
      {
        type: {
          imageUrl:
            "http://mak3r.com/wp-content/uploads/2014/09/wolverine-prosthetic-744x472.png",
          explication:
            "En médecine, on peut désormais fabriquer des prothèses au plus près des besoins du patient, avec quelques retouches personnelles !"
        },
        content: "Médecine"
      },

      {
        type: {
          imageUrl:
            "https://www.wedemain.fr/photo/art/grande/7401642-11401596.jpg?v=1422455243",
          explication:
            "Il est aussi possible de bâtir des maisons, même des immeubles !"
        },
        content: "Construction de batiments"
      },
      {
        type: {
          imageUrl:
            "https://www.ouest-france.fr/sites/default/files/styles/image-640x360/public/2016/05/20/une-moto-entierement-fabriquee-par-une-imprimante-3d_1.jpg?itok=6LX_ZpIP",
          explication:
            "Il existe des projets de fabrications de véhicules. Comme des motos entièrement réalisées en impression 3D !"
        },
        content: "Transports"
      }
    ]
  }
];

export const questionQuizLaser = [
  {
    id: 1,
    question:
      "Cette machine coupe: bois, plexiglas, et grave: verre et cuir grâce à ?",
    answers: [
      {
        type: {
          imageUrl:
            "http://www.bourasseau-industrie.fr/files/bourasseau/galerie-photos/page%20paris/decoupe_laser_paris.jpg",
          explication:
            "La découpe laser est un procédé de fabrication qui consiste à découper la matière grâce à une grande quantité d'énergie générée par un laser et concentrée sur une très faible surface"
        },
        content: "Un laser"
      },

      {
        type: {
          imageUrl:
            "http://www.avenirmeca.fr/wp-content/uploads/2015/06/decoupe-laser.jpg",
          explication:
            "La focalisation d'un rayon laser permet d'élever la température d'une zone réduite de matière, jusqu'à vaporisation. La puissance d'un laser varie en fonction du matériau à découper et de son épaisseur."
        },
        content: "une température élevée"
      },
      {
        type: {
          imageUrl: "https://www.fablablapalisse.fr/images/Photos/IMG_4838.JPG",
          explication:
            "Le choix de l'oxygène ou de l'azote pour la découpe laser en fonction des critères de qualité exigés pour l'application. "
        },
        content: "un gaz"
      }
    ]
  },
  {
    id: 2,
    question: "Cette machine s'utilise avec ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://cprint-sourcing.news/wp-content/uploads/2017/05/lucent-matiere-a-decoupe-4.jpg",
          explication:
            "vous pouvez découper, graver et marquer au laser de nombreuses matières : plastiques, bois, caoutchouc, cuir, métaux, textiles et bien plus encore"
        },
        content: "De la matière "
      },

      {
        type: {
          imageUrl:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSmRqd2Tv4t4Pxu2M3NYS-l2FlVaezKlB40SPXmPuToP8cowVxN",
          explication:
            "Extracteur filtreur de fumée pour machines laser ... Pour la gravure et la découpe laser de produits dégageant des fumées"
        },
        content: "Un extracteur de fumée et/ou un filtre "
      },
      {
        type: {
          imageUrl:
            "http://www.hellopro.fr/images/produit-2/4/1/0/machine-de-gravure-et-d-coupe-laser-co2-s-rie-g-6191014.jpg",
          explication:
            " Les couverclesg de protection agissent comme une protection pour le laser."
        },
        content: "Un couvercle traité"
      }
    ]
  },
  {
    id: 3,
    question: "Que peut-on fabriquer à l'aide d'un découpeuse laser ?",
    answers: [
      {
        type: {
          imageUrl:
            "http://agenda.reims.fr/data/c_2/i_15254/images/main/cabanesoiseaux.jpg",
          explication:
            "Si vous aimez les oiseaux, Saint-ex vous offre la possibilité de leurs fabriquer un petit nid douillet, sans le moindre effort, merci la découpeuse laser !"
        },
        content: "Une cabane à oiseaux"
      },

      {
        type: {
          imageUrl:
            "http://www.fabalpes.org/wp-content/uploads/2015/12/P51203-091443.jpg",
          explication:
            "Le carton peut être utilisé dans une découpeuse laser. on découpe les pièces une par une, puis on les assemble !"
        },
        content: "Un joli Cerf"
      },
      {
        type: {
          imageUrl:
            "http://www.eco2net.com/wp-content/uploads/2014/10/decoupe-laser1.jpg",
          explication:
            "Pas besoin d'assemblage, une plaque de plexiglas, une découpe, et hop, un chat"
        },
        content: "Un chat"
      }
    ]
  }
];

export const questionQuizLightPainting = [
  {
    id: 1,
    question: "Qu'est ce que le light painting ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://s23527.pcdn.co/wp-content/uploads/2017/11/Screenshot-2017-11-17-11.21.02.jpg.optimal.jpg",
          explication:
            "Le Lightpainting est une technique de prise de vue photographique basé sur la captation de la lumière"
        },
        content: "Un art moderne"
      },

      {
        type: {
          imageUrl:
            "https://www.histoiresdetongs.com/wp-content/uploads/2018/03/light-painting-tutoriel.jpg",
          explication:
            "La technique du light painting vous offre des possibilités créatives incroyables et infinies"
        },
        content: "Une représentation de formes lumineuses"
      },
      {
        type: {
          imageUrl:
            "https://cdn-files.cloud/wp-content/blogs.dir/66/files/2015/11/header-1.jpg",
          explication:
            "En effet, le LightPainting peut également représenter un mot, une phrase"
        },
        content: "Une représentation de caligraphie lumineuse"
      }
    ]
  },
  {
    id: 2,
    question:
      "Quels équipements sont primordiaux pour la pratique du light painting ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://www.ma-lampe-torche.com/wp-content/uploads/2017/06/lightpainting-intro.jpg",
          explication:
            "Tu peux faire du LightPainting avec une simple lampe torche ou avec un bâton lumineux spécial pour la pratique du light painting, même un sabre laser !"
        },
        content: "Lampes de poche"
      },

      {
        type: {
          imageUrl:
            "http://www.virusphoto.com/imagehosting/5734715d859cae7e.jpg",
          explication:
            "L'appareil photo est indispensable, c'est le support qui te permettra de capter la lumière"
        },
        content: "Appareil photo"
      },
      {
        type: {
          imageUrl:
            "https://www.canon.fr/Images/paiting-with-light-simple-shot_tcm79-1347646.jpg",
          explication:
            "Pour que la lumière soit la plus voyante possible, il faut que tu réalises le LightPainting dans une pièce sombre"
        },
        content: "Une pièce sombre"
      }
    ]
  },
  {
    id: 3,
    question: "Ou peut-tu faire du LightPainting ?",
    answers: [
      {
        type: {
          imageUrl:
            "http://media.topito.com/wp-content/uploads/2012/05/Light-Painting036.jpg",
          explication:
            "Si tu as un appareil photo avec un temps de pause de 30s mini, de la lumière et une pièce sombre, tu es un LightPainter !"
        },
        content: "Chez toi"
      },

      {
        type: {
          imageUrl:
            "http://www.saintex-reims.com/wp-content/uploads/2016/10/Light-Painting-Infinity.jpg",
          explication:
            "Saint-Ex offre la possibilité de découvrir le LightPainting, débloque la vidéo en bas pour en savoir plus !"
        },
        content: "A Saint-Ex !"
      },
      {
        type: {
          imageUrl:
            "http://media.topito.com/wp-content/uploads/2012/05/Light-Painting030.jpg",
          explication:
            "De préfèrence de nuit, avec appareil photo et lampe torche. Attention de ne pas sortir tout seul !"
        },
        content: "En extérieur"
      }
    ]
  }
];

export const questionQuizEscapeGames = [
  {
    id: 1,
    question: "C'est quoi un escape game ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://images.theconversation.com/files/214051/original/file-20180410-566-yzk5hc.jpg?ixlib=rb-1.1.0&rect=11%2C2%2C1577%2C807&q=45&auto=format&w=926&fit=clip",
          explication:
            "Le jeu d'évasion consiste la plupart du temps à parvenir à s’échapper d’une pièce dans une durée limitée (généralement une heure) et se pratique habituellement en groupe de plusieurs personnes"
        },
        content: "Les participants sont enfermés à l’intérieur d’une salle"
      },

      {
        type: {
          imageUrl:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/86th_FSS_unveils_Port_Royal_Mystery_Escape_Room_160621-F-ZC075-096.jpg/1024px-86th_FSS_unveils_Port_Royal_Mystery_Escape_Room_160621-F-ZC075-096.jpg",
          explication:
            "les joueurs doivent chercher des indices cachés dans une pièce, puis les combiner entre eux pour pouvoir avancer dans l’énigme"
        },
        content: "Les participants doivent trouver des énigmes à résoudre"
      },
      {
        type: {
          imageUrl:
            "http://www.femina.ch/sites/default/files/styles/portrait__750x500_/public/sablier-temps-ecouler-sable-escape-game-decompte-room-jeux.jpg?itok=bFiftu3T",
          explication:
            "Le décompte sera lancé dès votre entrée dans l'escape room, si vous ne sortez pas à temps ce n'est pas très grave en fait, le principal est d'aller le plus loin possible dans les énigmes pour découvrir les surprises de cette pièce !"
        },
        content:
          "Les participants doivent tenter de sortir en moins de 60 minutes ?"
      }
    ]
  },
  {
    id: 2,
    question: "Comment concevoir les énigmes d’un Escape Game ?",
    answers: [
      {
        type: {
          imageUrl:
            "https://static.independent.co.uk/s3fs-public/thumbnails/image/2018/02/21/17/escape-room.jpg?w968h681",
          explication:
            "Les énigmes peuvent être de simples manipulations d’objets (trouver une clé dans un tiroir qui permet d’ouvrir un coffre ou une porte)"
        },
        content:
          "Varier le type d’énigme : codes avec chiffres / codes avec lettres "
      },

      {
        type: {
          imageUrl:
            "https://escape-kit.com/wp-content/uploads/2017/01/e4aa8ed_17040-1fko2ho-300x199.jpg",
          explication:
            "Pour y réussir vous devrez résoudre fouiller, trouver des objets, des indices, résoudre des énigmes cachées dans la pièce"
        },
        content:
          "Cacher les éléments : les énigmes doivent être cachés, mais pas trop"
      },
      {
        type: {
          imageUrl:
            "https://france3-regions.francetvinfo.fr/bourgogne-franche-comte/sites/regions_france3/files/styles/top_big/public/assets/images/2018/04/25/live-escape-game-1155620_1920-3626707.jpg?itok=pT2h3Pta",
          explication:
            "Certaines escape games peuvent acceuillir 10 joueurs en même temps"
        },
        content:
          "Le nombre d’énigmes à proposer dépend du nombre de participants, du temps disponible, de leur complexité"
      }
    ]
  },
  {
    id: 3,
    question: "Comment créer des effets spéciaux",
    answers: [
      {
        type: {
          imageUrl: "https://i.ytimg.com/vi/zkwKMjWC2CU/hqdefault.jpg",
          explication:
            "Mettre en place un musique d’ambiance. Sinistre. Une playlist par exemple. Ou des sons. Liés à votre scénario"
        },
        content: "Diffuser une musique d’ambiance (stressante, sinistre…)."
      },

      {
        type: {
          imageUrl:
            "https://img.grouponcdn.com/deal/23BRDPDafWNDFQhvta4qjpBbFpkh/23-700x420/v1/c700x420.jpg",
          explication:
            "Il faut jouez avec l’éclairage. Un grand classique. la lumière « noire », des UV. Voire même avec des lasers"
        },
        content:
          "L’éclairage (ou l’absence d’éclairage) contribue à l’ambiance."
      },
      {
        type: {
          imageUrl:
            "https://www.sortiraparis.com/images/62/69688/399010-un-escape-game-pirate-pour-les-enfants-a-puteaux-2.jpg",
          explication:
            "Soyez prêts à enfiler votre plus beau costume d'aventurier!"
        },
        content:
          "Maquiller les participants et les déguiser pour rendre l’aventure encore plus réaliste"
      }
    ]
  }
];
